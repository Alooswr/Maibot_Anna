...
class NapCatEventRouter:
    ...
    async def handle_inbound_message(self, payload: NapCatPayloadDict) -> None:
        ...
        if not self_id:
            self_id = str(getattr(runtime.runtime_state, "_reported_account_id", "") or "").strip()
        if self_id:
        ...
        if settings.filters.ignore_self_message and self._is_self_message(payload, sender, self_id, sender_user_id):
            return
    ...
    def _is_self_message(
        ...
        return normalized_self_id in candidate_ids
