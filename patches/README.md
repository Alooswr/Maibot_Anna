...
线上版本做过安全修正：不再让 `<reject>` / `<veto>` 这类内部控制标记穿透到可见回复路径，并在连续 veto 场景下清理响应。当前没有可靠的原始基线文件，因此这里暂不放 diff，避免生成无法干净应用的补丁。

## MaiBot-Napcat-Adapter

线上版本修过私聊自身回显过滤：判断自身消息时不能只看顶层 `user_id`，还要比较 `sender.user_id`、`sender.uin` 和当前 `self_id`。当前没有可靠的原始基线文件，因此这里暂不放 diff。

如果要把这两项也公开给群友，建议单独 fork 对应上游插件仓库，从上游当前版本重新整理一版最小 patch/PR。
